...
import { pasaportGecerliMi } from '@/admin/baslat-menusu/erp/cari/cariFormatYardimci';
...
export function cariVergiNoGecerliMi(deger: string, isletmeTuru: string): boolean {
  const v = deger.trim();
  if (!v) return true;
  if (isletmeTuru === 'YABANCI') return pasaportGecerliMi(v);
...
export function cariFormDogrula(form: CariFormDegeri): string | null {
  ...
  if (form.isletmeTuru && !['TUZEL', 'GERCEK', 'YABANCI'].includes(form.isletmeTuru) && form.isletmeTuru.length > 40) {
    return 'İşletme türü çok uzun';
  }
  if (!cariVergiNoGecerliMi(form.vergiNo, form.isletmeTuru)) {
    if (form.isletmeTuru === 'YABANCI') {
      return 'Pasaport no 5-20 karakter olmalıdır (harf ve rakam)';
    }
