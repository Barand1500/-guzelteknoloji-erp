...
import {
  KART_TIPLERI_GUNCELLENDI,
  kartTipiEkle,
  kartTipiGuncelle,
  kartTipiSil,
  kartTipleriGetir,
  type KartTipi,
} from '@/admin/baslat-menusu/ozel-tanimlar/veri/kartTipleri';
import {
...
export function KartTipleriListeSayfasi() {
  ...
  const [duzenlenen, setDuzenlenen] = useState<KartTipi | null>(null);
  const [silinecek, setSilinecek] = useState<KartTipi | null>(null);
  const [adi, setAdi] = useState('');
  const [hata, setHata] = useState('');

  const yenile = useCallback(() => setListe(kartTipleriGetir()), []);

  useEffect(() => {
    const h = () => yenile();
    window.addEventListener(KART_TIPLERI_GUNCELLENDI, h);
    return () => window.removeEventListener(KART_TIPLERI_GUNCELLENDI, h);
  }, [yenile]);
  ...
  function kaydet(e: FormEvent) {
    e.preventDefault();
    if (duzenlenen) {
      if (!kartTipiGuncelle(duzenlenen.id, { adi, aktif: true })) {
        setHata('Güncellenemedi. Ad benzersiz olmalı.');
        return;
      }
    } else if (!kartTipiEkle({ adi, aktif: true })) {
        ...
        footer={
          <SistemModalAksiyonlar>
            <div className="flex justify-end gap-2">
              ...
              <button type="submit" form="ot-kt-form" className="ot-btn-kaydet">
                Kaydet
              </button>
            </div>
          </SistemModalAksiyonlar>
