...
export function CariTipleriListeSayfasi() {
  const [liste, setListe] = useState<CariTipi[]>(() => cariTipleriGetir());
  const [arama, setArama] = useState('');
  const [sayfaBoyutu, setSayfaBoyutu] = useState(10);
  const [sayfa, setSayfa] = useState(1);
  const [modalAcik, setModalAcik] = useState(false);
  const [duzenlenen, setDuzenlenen] = useState<CariTipi | null>(null);
  const [silinecek, setSilinecek] = useState<CariTipi | null>(null);
  const [adi, setAdi] = useState('');
  const [hata, setHata] = useState('');

  const yenile = useCallback(() => setListe(cariTipleriGetir()), []);

  useEffect(() => {
    const h = () => yenile();
    window.addEventListener(CARI_TIPLERI_GUNCELLENDI, h);
    return () => window.removeEventListener(CARI_TIPLERI_GUNCELLENDI, h);
  }, [yenile]);
  ...
  function kaydet(e: FormEvent) {
    e.preventDefault();
    if (duzenlenen) {
      if (!cariTipiGuncelle(duzenlenen.id, { adi, aktif: true })) {
        setHata('Güncellenemedi. Ad benzersiz olmalı.');
        return;
      }
    } else if (!cariTipiEkle({ adi, aktif: true })) {
  ...
  return (
    <div className="ot-pb-sayfa">
        ...
        footer={
          <SistemModalAksiyonlar>
            <div className="flex justify-end gap-2">
              ...
              <button type="submit" form="ot-ct-form" className="ot-btn-kaydet">
                Kaydet
              </button>
            </div>
          </SistemModalAksiyonlar>
      ...
      >
        <form id="ot-ct-form" className="ot-pb-form" onSubmit={kaydet}>
