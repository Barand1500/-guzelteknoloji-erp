...
export function StokTipleriListeSayfasi() {
  const [liste, setListe] = useState<StokTipiOt[]>(() => stokTipleriOtGetir());
  const [arama, setArama] = useState('');
  const [sayfaBoyutu, setSayfaBoyutu] = useState(10);
  const [sayfa, setSayfa] = useState(1);
  const [modalAcik, setModalAcik] = useState(false);
  const [duzenlenen, setDuzenlenen] = useState<StokTipiOt | null>(null);
  const [silinecek, setSilinecek] = useState<StokTipiOt | null>(null);
  const [adi, setAdi] = useState('');
  const [hata, setHata] = useState('');

  const yenile = useCallback(() => setListe(stokTipleriOtGetir()), []);

  useEffect(() => {
    const h = () => yenile();
    window.addEventListener(STOK_TIPLERI_OT_GUNCELLENDI, h);
    return () => window.removeEventListener(STOK_TIPLERI_OT_GUNCELLENDI, h);
  }, [yenile]);
  ...
  const filtrelenen = useMemo(() => {
    ...
    if (!q) return liste;
    return liste.filter((t) => t.adi.toLocaleLowerCase('tr').includes(q));
  }, [liste, arama]);
  ...
  function kaydet(e: FormEvent) {
    e.preventDefault();
    if (duzenlenen) {
      if (!stokTipiOtGuncelle(duzenlenen.id, { adi, aktif: true })) {
        setHata('Güncellenemedi. Ad benzersiz olmalı.');
        return;
      }
    } else if (!stokTipiOtEkle({ adi, aktif: true })) {
  ...
  return (
    <div className="ot-pb-sayfa">
        ...
        footer={
          <SistemModalAksiyonlar>
            <div className="flex justify-end gap-2">
              ...
              <button type="submit" form="ot-st-form" className="ot-btn-kaydet">
                Kaydet
              </button>
            </div>
          </SistemModalAksiyonlar>
      ...
      >
        <form id="ot-st-form" className="ot-pb-form" onSubmit={kaydet}>
