...
import { offlinePanelDeposuTemizle } from './offlinePanelDepo';
...
export async function adminJsonFetch<T>(path: string, init?: RequestInit): Promise<T> {
  ...
  if (offlineMock) {
    if (import.meta.env.PROD) {
      throw new Error(
        'Panel offline modda calisiyor (mock). Sunucuda ./deploy.sh calistirin ve tarayicida Ctrl+Shift+R yapin.'
      );
    }
